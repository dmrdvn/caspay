...
export function HomePricing({ sx, ...other }: BoxProps) {
  ...
  const renderDescription = () => (
    <SectionTitle
      caption="pricing"
      title="Simple &"
      txtGradient="Transparent"
      description="Start free, scale as you grow. No hidden fees, no surprises. Pay only for what you use."
...
function PlanCard({ plan, sx, ...other }: PlanCardProps) {
  const standardLicense = plan.license === 'Free';
            ...
            <Box component="span" sx={{ typography: 'h3' }}>
              {typeof plan.price === 'number' ? (
                plan.price === 0 ? (
                  ...
                  <>
                    {plan.price}%<Box component="small" sx={{ typography: 'body2', ml: 0.5 }}>per transaction</Box>
                  </>
                )
              ) : (
                plan.price
