...
type Props = CustomToolbarSettingsButtonProps & {
  ...
  options: {
    stocks: FilterOption[];
    publishs: FilterOption[]; // Keep name for backward compat, but uses 'active' filter
  };
...
export function ProductTableToolbar({
  ...
  const [active, setActive] = useState<string[]>(currentFilters.active || []);
  ...
  const renderLeftPanel = () => (
    <>
      ...
      <FilterSelect
        ...
        onChange={handleSelect(setActive)}
        onApply={() => updateFilters({ active })}
      />
