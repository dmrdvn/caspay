...
export function MerchantWidgetSummary({ title, percent, total, chart, sx, ...other }: Props) {
  ...
  // Format total based on title (if it's subscriptions, don't use currency)
  const formattedTotal = title.toLowerCase().includes('subscription') && !title.toLowerCase().includes('revenue')
    ? fNumber(total)
    : fCurrency(total);
  ...
  return (
    ...
    >
      <Box sx={{ flexGrow: 1 }}>
        ...
        <Box sx={{ my: 1.5, typography: 'h3' }}>{formattedTotal}</Box>
